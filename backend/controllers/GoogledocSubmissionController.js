...
const getVivavoce = async(req,res) => {
  ...
  try {
    ...
    if (vivavocesubmission) {
      res.status(200).json({message: 'Report already submitted', submitted: true, finalreport: true});
    } else {
      const finalreport = await ReportData.findOne({username: email})
      if (finalreport) {
        res.status(200).json({message: 'Final report already submitted', submitted: false, finalreport: true})
      }else{
        res.status(200).json({message: 'Report submission pending', submitted: false, finalreport: false});
      }
    }
